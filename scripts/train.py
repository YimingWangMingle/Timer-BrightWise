...
from tsfm.production_plan import ResolvedTrainingPlan
from tsfm.runtime import build_finite_s3_dataset,build_s3_dataset
...
def parse_args():
    ...
    p.add_argument("--resolved-plan",type=Path)
    p.add_argument("--steps",type=int);p.add_argument("--device",choices=("auto","cuda","cpu"),default="auto");return p.parse_args()

def main():
    a=parse_args();missing=[n for n in ("model_config","manifest","output_dir") if getattr(a,n) is None]
    if missing:raise ValueError(f"missing required arguments: {', '.join(missing)}")
    if (a.training_config is None)==(a.resolved_plan is None):raise ValueError("provide exactly one of --training-config or --resolved-plan")
    c=distributed_context_from_environment();mc=TimerConfig.from_json(a.model_config)
    plan=ResolvedTrainingPlan.from_json(a.resolved_plan) if a.resolved_plan else None
    tc=plan.training_config() if plan else TrainingConfig.from_json(a.training_config)
    plan_document=plan.to_dict() if plan else None
    if plan and plan.world_size!=c.world_size:raise ValueError("resolved plan world size differs from runtime")
    ...
    if plan:
        train,digest,window_count=build_finite_s3_dataset(a.manifest,split="train",seed=tc.seed,patch_length=mc.input_token_len,context_patches=tc.context_patches)
        if window_count!=plan.window_count:raise ValueError("resolved plan window count differs from manifest")
    else:train,digest=build_s3_dataset(a.manifest,split="train",seed=tc.seed,patch_length=mc.input_token_len,context_patches=tc.context_patches)
    snapshots={"model":asdict(mc),"data":{"manifest_checksum":digest},"training":tc.to_dict()}
    if plan_document is not None:snapshots["resolved_plan"]=plan_document
    if a.action=="probe":
    ...
    if a.action=="run":
        ...
        if a.steps is not None:
            report=run_training(model=model,dataset=train,training_config=tc,output_dir=a.output_dir,manifest_checksum=digest,config_snapshots=snapshots,device=device,resume=a.resume,total_steps_override=a.steps,rank=c.rank,world_size=c.world_size,is_main_process=c.is_main_process,resolved_plan=plan_document)
        else:report=run_validated_training(model=model,train_dataset=train,validation_datasets={"val_heldout":heldout,"val_temporal":temporal},training_config=tc,output_dir=a.output_dir,manifest_checksum=digest,config_snapshots=snapshots,device=device,resume=a.resume,rank=c.rank,world_size=c.world_size,is_main_process=c.is_main_process,resolved_plan=plan_document)
    else:
        ...
        report=run_training(model=model,dataset=dataset,training_config=tc,output_dir=a.output_dir,manifest_checksum=digest,config_snapshots=snapshots,device=device,resume=a.resume,total_steps_override=target,rank=c.rank,world_size=c.world_size,is_main_process=c.is_main_process,resolved_plan=plan_document)
